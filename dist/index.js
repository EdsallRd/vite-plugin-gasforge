...
if (typeof globalThis.URLSearchParams === "undefined") globalThis.URLSearchParams = class URLSearchParams {};`,
    ...
    watchChange(id) {
      if (/\.(ts|tsx)$/.test(id) && !id.endsWith(".gen.ts") && !id.endsWith("gasforge-virtual.d.ts")) {
...
function generateDeclarationFile(root, registry) {
  if (registry.length === 0) {
...
function writeDeclarationFile(root, registry) {
  const content = generateDeclarationFile(root, registry);
  writeFileSync(resolve3(root, "gasforge-virtual.d.ts"), content, "utf-8");
}
